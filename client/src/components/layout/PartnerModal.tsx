...
import { CheckCircle2, X, Users, ArrowRight, MessageSquare, Globe } from 'lucide-react';
          ...
          >
            {success ? (
              <div className="p-16 text-center">
                ...
                <h2 className="text-3xl font-black text-[#1A1F2C] mb-4">Partnership Initiated!</h2>
                <p className="text-slate-500 font-medium">Thank you for your interest in the Europack Ecosystem. Our partnership team will review your requirement and reach out shortly.</p>
              </div>
            ) : (
              <div className="flex flex-col md:flex-row min-h-full">
                <div className="w-full md:w-2/5 bg-[#1A1F2C] p-10 text-white flex flex-col justify-between relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="inline-block px-3 py-1 rounded-full bg-[#FF6600]/20 text-[#FF6600] text-[10px] font-black uppercase tracking-widest mb-6">Ecosystem Growth</div>
                    <h3 className="text-3xl font-black leading-tight mb-4">Partner With <br /><span className="text-[#FF6600]">Europack.</span></h3>
                    <p className="text-slate-400 text-sm leading-relaxed mb-8">Join India's leading industrial packaging network as a vendor or contractor.</p>
                    <div className="space-y-4">
                      <div className="flex items-center gap-3 text-xs font-bold text-slate-300">
                        <Globe size={14} className="text-[#FF6600]" /> Pan-India Operations
                      </div>
                      <div className="flex items-center gap-3 text-xs font-bold text-slate-300">
                         <MessageSquare size={14} className="text-[#FF6600]" /> Expert Support
                      </div>
                    </div>
                  </div>
                  <div className="mt-12 text-[10px] font-black uppercase tracking-[0.2em] text-[#FF6600]">Expand Your Horizon</div>
                </div>
                <div className="w-full md:w-3/5 p-10">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Partnership Requirements</label>
                      <textarea required rows={4} value={formData.requirement} onChange={e => setFormData({ ...formData, requirement: e.target.value })} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-[#FF6600] outline-none transition-all text-sm font-bold resize-none" placeholder="Vendor services, channel sales, or contracting..." />
                    </div>
                    <button disabled={loading} type="submit" className="w-full py-5 rounded-2xl bg-[#FF6600] text-white font-black uppercase tracking-widest text-xs hover:bg-[#CC5200] transition-all flex items-center justify-center gap-3 shadow-lg shadow-orange-100 active:scale-[0.98]">
                      {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <>Request Partnership <ArrowRight size={16}/></>}
                    </button>
                  </form>
                </div>
              </div>
